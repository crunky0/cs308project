...
async def create_order_with_invoice(order_data: OrderRequest):
    """
    Endpoint to create an order, generate an invoice, and send it via email.
    Also returns the invoice content to display it on the screen.
    """
    try:
        ...
        # Step 2: Fetch user email and name from the database
        user = await database.fetch_one("SELECT name, email FROM users WHERE userid = :userid", {"userid": order_data.userid})
        if not user:
        ...
        invoice_html = invoice_service._create_invoice_html(
            ...
            user_info={"name": user_name, "email": email},
            items=items,
        ...
        # Step 6: Return the invoice HTML to display on the screen
